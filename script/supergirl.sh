...
game_conf(){
	...
	sed -i "s/Server.manager.port=10000/Server.manager.port=$[10000+${1}]/" ${game_conf_file}
	sed -i "s/supergirl_0/supergirl_${1}/" ${game_conf_file}
...
mysql_conf(){
	create_sql_file=/data/server/s0/bin/superGirl_new.sql
	update_sql_file=/data/server/s0/bin/superGirl_update.sql
	if [ -s ${create_sql_file} ];then
		# TODO 封装MySQL操作 
		/usr/local/mysql/bin/mysql -uroot -p'DFDF900((#$$roo@' -e "create database supergirl_$1;use supergirl_$1;source ${create_sql_file};"
		/usr/local/mysql/bin/mysql -uroot -p'DFDF900((#$$roo@' -e "GRANT ALL ON supergirl_$1.* to ppgames@'%' IDENTIFIED BY 'd7dfdfyhx0jjAidd888';FLUSH PRIVILEGES;"
	fi
	if [ -s ${update_sql_file} ];then
		#/usr/local/mysql/bin/mysql -uroot -p'DFDF900((#$$roo@' -e "use supergirl_$1;source ${update_sql_file};"
...
gen_new_sid(){
	maxsid=$(/usr/local/mysql/bin/mysql -uppgames -p'IDFS(7^%x#2' -h10.168.79.161 supergirlserver -sse "SELECT MAX(sid) msid FROM t_server")
	if [ "${maxsid}" == "NULL" ]; then
		maxsid=100;
	else
		maxsid=$[$maxsid+1]
	...
	/usr/local/mysql/bin/mysql -uppgames -p'IDFS(7^%x#2' -h10.168.79.161 supergirlserver -e "${insert_sql}" || die "gen_new_sid fail"
	return ${maxsid}
}
