...
mysql_conf(){
	...
	if [ -s ${create_sql_file} ];then
		# TODO 封装MySQL操作 
		/usr/local/mysql/bin/mysql -uroot -p'DFDF900((#$$roo@' -e "create database demo_$1;use demo_$1;source ${create_sql_file};"
		/usr/local/mysql/bin/mysql -uroot -p'DFDF900((#$$roo@' -e "GRANT ALL ON demo_$1.* to ppgames@'%' IDENTIFIED BY 'd7dfdfyhx0jjAidd888';FLUSH PRIVILEGES;"
	fi
	if [ -s ${update_sql_file} ];then
		#/usr/local/mysql/bin/mysql -uroot -p'DFDF900((#$$roo@' -e "use demo_$1;source ${update_sql_file};"
...
gen_new_sid(){
	maxsid=$(/usr/local/mysql/bin/mysql -uppgames -p'IDFS(7^%x#2' -h10.168.79.161 demoserver -sse "SELECT MAX(sid) msid FROM t_server")
	if [ "${maxsid}" == "NULL" ]; then
		maxsid=100;
	else
		maxsid=$[$maxsid+1]
	...
	/usr/local/mysql/bin/mysql -uppgames -p'IDFS(7^%x#2' -h10.168.79.161 demoserver -e "${insert_sql}" || die "gen_new_sid fail"
	return ${maxsid}
}
